...
console.log(incr.then(incr).f(1))
...
let map_Pair = <a, a1, b, b1>(f: Fun<a, a1>, g: Fun<b, b1>):
    ...
    return Fun((p: Pair<a, b>) => {
        ...
        let a1 = f.f(p.fst)
        let b1 = g.f(p.snd)
        return pair<a1, b1>(a1, b1)
    })
}
let pair = function <a, b>(first: a, second: b): Pair<a, b> {
    return { fst: first, snd: second }
}

// See sultion 2
console.log(map_Pair(incr, double).f(pair(10, 10)))
